...
from forms import AddProduct, AddOrderDetails
...
def checkout(): 
    add_form= AddOrderDetails()
    if add_form.validate_on_submit():

        total_price = sum(float(item['item_price']) for item in items_in_cart)
        # Create a new order record
        current_user= User()
        if current_user.is_authenticated and current_user.id:
            # Create a new order record
            order = Order.create(user_id=current_user.id, order_date=datetime.now(), order_amount=total_price)

        # Then, for each item in the cart, create an OrderDetail record
            for item in items_in_cart:
                OrderDetail.create(
                    order_id=order.id,
                    product_id=item['item_product_id'],
                    address=add_form.address.data,
                    pincode=add_form.pincode.data,
                    city= add_form.city.data,
                    contact_no= add_form.contact_no.data,
                    )
        return redirect(url_for('success'))
    return render_template('checkout.html', form=add_form)
...
#--------------------------- ITEM DISCRIPTION ---------------------------------#
@app.route('/item_description/<int:id>')
